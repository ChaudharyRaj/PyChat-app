    ...
    if request.method == 'POST' and form.validate():
        ...
        if result > 0:
            ...
            if sha256_crypt.verify(password_candidate, password):
                ...
                session['s_name'] = fullname
                session['uid'] = uid
                x = '1'
                id ='uid'
                cur.execute("UPDATE users SET online=%s WHERE id=%s", (x, uid))
                flash('You are now logged in', 'success')
                return redirect(url_for('chatting', id=session['uid']))
            else:
...
def chatting(id):
    if 'uid' in session:
        ...
        if get_result > 0:
            ...
            # Close Connection
            cur.close()
            return render_template('chating.html', users=users, form=form)
        else:
